...
async function initApp() {
    ...
    document.addEventListener('visibilitychange', async () => {
        if (!document.hidden) {
            try {
                console.log('👁️ Пользователь вернулся в приложение - синхронизируем данные');
                await syncLoyaltyWithServer();
            } catch (error) {
                console.error('❌ Ошибка синхронизации при возвращении:', error);
            }
        }
    });
